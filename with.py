...
class AdStripper:
    ...
    def response(self, flow: http.HTTPFlow) -> None:
        ...
        if flow.response.status_code == 206 and "audio" in content_type:
            ...
            if self.url in self.stripped.keys():
                ...
                d, tor_length = self.strip_ads(data)
    ...
    def strip_ads(self, data: bytes) -> tuple[bytes, int]:
        d = b""
        prev_idx = 0
        i = 0
        try:
            with self.session.get(self.url, stream=True) as x:
                try:
                    l = int(x.headers["Content-Length"])
                    n = int(l / self.delta)
                except KeyError:
                    n = "?"
                ...
                for chunk in x.iter_content(self.delta):
                    ...
                    idx = data.find(chunk)
                    if idx != -1 and abs(idx-prev_idx) <= 50*self.delta:
                        d += chunk
